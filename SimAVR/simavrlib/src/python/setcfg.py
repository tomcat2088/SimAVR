...
def readArgs():
  ...
  p.add_option("-o", dest = "my_option", help = "my_option name for section in setup.cfg")
  p.add_option("-s", dest = "value", help = "value to set for section/my_option in setup.cfg")
  opts, args = p.parse_args()
  if len(args) > 0 : p.error("to much arguments, none expected")
  if opts.section is None or opts.my_option is None or opts.value is None:
...
def writeCfg(opts):
  ...
  cfg.set(opts.section, opts.my_option, opts.value)
  try:
